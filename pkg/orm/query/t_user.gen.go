...
func newUser(db *gorm.DB) user {
	...
	_user.ClassID = field.NewString(tableName, "class_id")
	_user.Class = userBelongsToClass{
...
func (u *user) updateTableName(table string) *user {
	...
	u.ClassID = field.NewString(table, "class_id")
...
type userBelongsToClass struct {
	db *gorm.DB

	field.RelationField
}

func (a userBelongsToClass) Where(conds ...field.Expr) *userBelongsToClass {
	if len(conds) == 0 {
		return &a
	}
...
func (a userBelongsToClass) WithContext(ctx context.Context) *userBelongsToClass {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a userBelongsToClass) Model(m *model.User) *userBelongsToClassTx {
	return &userBelongsToClassTx{a.db.Model(m).Association(a.Name())}
}

type userBelongsToClassTx struct{ tx *gorm.Association }

func (a userBelongsToClassTx) Find() (result *model.Class, err error) {
	return result, a.tx.Find(&result)
}

func (a userBelongsToClassTx) Append(values ...*model.Class) (err error) {
...
func (a userBelongsToClassTx) Replace(values ...*model.Class) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userBelongsToClassTx) Delete(values ...*model.Class) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userBelongsToClassTx) Clear() error {
	return a.tx.Clear()
}

func (a userBelongsToClassTx) Count() int64 {
	return a.tx.Count()
}
